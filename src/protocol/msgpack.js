import {COMMAND_DEFAULT_FORMATS, COMMAND_FIELDS, decoder, encoder} from "../constants.js";
import {concatBytes, toUint8Array, valuesEqual} from "./binary.js";
...
function decodeValue(bytes, offset = 0) {
    ...
    if (head >= 0xe0) return {
        ...
        offset,
        raw: bytes.slice(offset - 1, offset)
    };
    if ((head & 0xf0) === 0x80) {
        ...
        return {
            ...
            format: "fixmap",
            entries,
            headerBytes: bytes.slice(offset - 1 - (count * 2), offset),
            offset,
            raw: bytes.slice(offset - (offset - (offset - 1)), offset)
        };
    }
    if ((head & 0xf0) === 0x90) {
        const count = head & 0x0f;
    ...
    switch (head) {
        ...
        case 0xca:
            return {
                value: new DataView(bytes.buffer, bytes.byteOffset + offset, 4).getFloat32(0, false),
                format: "float32",
                offset: offset + 4,
        ...
        case 0xcb:
            return {
                value: new DataView(bytes.buffer, bytes.byteOffset + offset, 8).getFloat64(0, false),
                format: "float64",
                offset: offset + 8,
                raw: bytes.slice(offset - 1, offset + 8)
            };
        case 0xcc:
            return {
                value: bytes[offset],
                format: "uint8",
                offset: offset + 1,
                raw: bytes.slice(offset - 1, offset + 1)
            };
        case 0xcd:
            return {
                ...
                format: "uint16",
                offset: offset + 2,
                raw: bytes.slice(offset - 1, offset + 2)
            };
        case 0xce:
            return {
        ...
        case 0xd1:
            return {
                value: new DataView(bytes.buffer, bytes.byteOffset + offset, 2).getInt16(0, false),
                format: "int16",
                offset: offset + 2,
                raw: bytes.slice(offset - 1, offset + 2)
            };
        case 0xd2:
            return {
                value: new DataView(bytes.buffer, bytes.byteOffset + offset, 4).getInt32(0, false),
                format: "int32",
                offset: offset + 4,
                raw: bytes.slice(offset - 1, offset + 4)
            };
...
function readFast(bytes) {
    ...
    switch (head) {
        ...
        case 0xc3:
            return true;
        case 0xc4:
            return readBinFast(bytes, bytes[cursor++]);
        case 0xc5: {
        ...
        case 0xd9:
            return readStrFast(bytes, bytes[cursor++]);
        case 0xda: {
            const len = (bytes[cursor] << 8) | bytes[cursor + 1];
            cursor += 2;
